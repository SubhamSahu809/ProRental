...
const UserAvatar = ({ user, onClick, showDropdownIcon = false, size = "md" }) => {
  ...
  if (user) {
    ...
    return (
      <button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer ${size === "sm" ? "p-1.5" : ""}`}
      >
  ...
  return (
    <button
      ...
      className={`${sizeClasses[size]} inline-flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer`}
      aria-label="Login"
    >
